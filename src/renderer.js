...
function buildReport(d, s) {
  ...
  let dl = d.deadline;
  try { dl = new Date(d.deadline + "T00:00:00").toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }); } catch (e) {}
  const row = (cells, th) => `<tr>${cells.map((c) => `<${th ? "th" : "td"}>${c}</${th ? "th" : "td"}>`).join("")}</tr>`;

  const jobRows = [...d.jobs].sort((a, b) => (a.date < b.date ? -1 : 1))
    .map((j) => `<tr><td>${j.date}</td><td>${cap1(j.type)}</td><td>${cap1(j.boiler || "&mdash;")}</td><td>${j.type === "repair" ? cap1(j.fault || "&mdash;") : "&mdash;"}</td><td>${(j.h ?? "") + " h"}</td><td>${j.engineer ? esc(j.engineer) : "&mdash;"}</td><td class="note">${j.notes ? esc(j.notes) : "&mdash;"}</td></tr>`).join("")
  ...
  const engRows = engineers.map((e) => {
    ...
    const hoursN = (d.hours || []).filter((h) => h.engineer === e.name).length;
    const meta = [e.regNo && `Reg ${esc(e.regNo)}`, e.licence && `Licence ${esc(e.licence)}`,
      e.company && esc(e.company),
      e.expiry && `card to ${(() => { try { return new Date(e.expiry + "T00:00:00").toLocaleDateString("en-GB", { month: "short", year: "numeric" }); } catch { return e.expiry; } })()}`
    ].filter(Boolean).join(" &middot; ");
    return row([esc(e.name), meta || "&mdash;", e.categories ? esc(e.categories) : "&mdash;", jobs, hoursN]);
  }).join("");
  const hourRows = [...d.hours].sort((a, b) => (a.date < b.date ? -1 : 1))
    .map((r) => `<tr><td>${r.date}</td><td>${(r.h ?? "") + " h"}</td><td>${r.engineer ? esc(r.engineer) : "&mdash;"}</td><td class="note">${r.note ? esc(r.note) : "&mdash;"}</td></tr>`).join("")
    || `<tr><td colspan="4" class="empty">No assisted hours logged yet</td></tr>`;

  const runLog = window.GasLogic.runningLog(d);
  const runLogRows = runLog.map((r) => r.kind === "base"
    ? `<tr><td>&mdash;</td><td><em>before this app</em></td><td>${r.h} h</td><td><b>${r.running} h</b></td></tr>`
    : `<tr><td>${r.date}</td><td>${r.n}</td><td>${r.h} h</td><td><b>${r.running} h</b></td></tr>`
  ...
  const portDate = s.portfolioCanFinish
    ? new Date(s.portfolioFinishDate + "T00:00:00").toLocaleDateString("en-GB", { day: "numeric", month: "short" })
    : "&mdash;";
    ...
    * { box-sizing: border-box; }
    ...
    <div class="sheet">
      ...
      ${engineers.length ? `<section>
        ...
        <table>${row(["Name", "Registration", "Categories", "Write-ups", "Assisted hours"], true)}${engRows}</table>
      </section>` : ""}
      ...
      <section>
        ...
        <table>${row(["Date", "Hours", "Assisted by", "What was the job?"], true)}${hourRows}</table>
      </section>

      <section>
        <h2>Job log &mdash; running total</h2>
        <table>${row(["Date", "Job #", "Hours", "Running total"], true)}${runLogRows}</table>
      </section>

      <footer>Generated by Gas Portfolio Tracker on ${gen}${name ? " for " + esc(name) : ""}. Figures are self-recorded &mdash; made by C. Wales.</footer>
    </div>
  </body></html>`;
}
...
// label a saved engineer name, flagging it if they've since been removed in Settings
function engineerLabel(name) {
  if (!name) return "";
  const gone = !(data.engineers || []).some((e) => e.name === name);
  return esc(name) + (gone ? ' <span class="dim sm">(removed)</span>' : "");
}
function engineerOptions(selected) {
  ...
  // the entry's engineer was since removed from Settings — keep it selectable so saving
  // the entry (e.g. editing a date) doesn't silently blank out who it was logged with
...
// books every weekday from fromISO to toISO (inclusive) as a day off
function addOff(fromISO, toISO_) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(fromISO || "")) { toast("pick a start date"); return; }
  const a = parseISO(fromISO), b = parseISO(toISO_ || fromISO);
  ...
  for (const dt = new Date(a); dt <= b; dt.setDate(dt.getDate() + 1)) {
    if (dt.getDay() === 0 || dt.getDay() === 6) continue;   // weekdays only
    const iso = toISO(dt);
...
function render() {
  ...
  else if (tab === "joblog") app.appendChild(jobLogPane(s));
...
// the next 14 days, flat day chips matching the rest of Home (.htile / .pip) —
// colour-coded by status, today gets a focus-ring border, logged days a small check
function dayStripHTML(s) {
  ...
  const segs = days.map((dy) => {
    const dt = window.GasLogic.parseISO(dy.date);
    return `<div class="dseg ${dy.status}${dy.today ? " today" : ""}" title="${esc(fmtDate(dy.date))}${dy.today ? " — today" : ""}">
        <span class="dseg-m">&#10003;</span>
        <span class="dseg-n">${dt.getDate()}</span>
        <span class="dseg-w">${dt.toLocaleDateString(undefined, { weekday: "short" }).slice(0, 2)}</span>
      </div>`;
  }).join("");

  return `<div class="daystrip">
    <div class="hcap">Next 14 days <span class="dim sm">${startLbl} &ndash; ${endLbl}</span></div>
    <div class="dsrow">${segs}</div>
    <div class="dslegend">
...
function hoursPane(s) {
  ...
  const recent = rowsAll.map(({ r, i }) => {
    ...
    return `<li><span class="li-d">${esc(r.date)}</span><span class="li-v">${r.h} h${r.engineer ? " &middot; " + engineerLabel(r.engineer) : ""}${r.note ? " &middot; " + esc(r.note) : ""}</span>` +
...
function writeupsPane(s) {
  ...
  const recent = rowsAll.map(({ r, i }) => {
    ...
    const eng = r.engineer ? ` &middot; ${engineerLabel(r.engineer)}` : "";
  ...
  el.innerHTML = `
    ...
    <p class="dim sm" style="margin:-6px 0 12px">"Unassisted" means <b>you</b> did the hands-on work &mdash; it still needs a Gas&nbsp;Safe registered engineer present and responsible on site. Pick who that was below.</p>
...
function progressPane(s) {
  ...
  const logTail = log.slice(-5).reverse();
  const logRows = logTail.map((r) => r.kind === "base"
    ...
    : `<div class="jlog-row"><span class="jlog-n">${r.n}</span><span class="jlog-d">${esc(fmtDate(r.date, { day: "2-digit", month: "short", year: "numeric" }))}</span><span class="jlog-h">${trimNum(r.h, 1)} h</span><span class="jlog-t">${trimNum(r.running, 1)} h</span></div>`
  ).join("");
  const logHTML = log.length
    ? `<div class="jlog"><div class="jlog-row jlog-head"><span class="jlog-n">#</span><span class="jlog-d">Date</span><span class="jlog-h">Hrs</span><span class="jlog-t">Running</span></div>${logRows}</div>
       <button class="pill go" data-goto="joblog">Open full job log &rarr;</button>`
  ...
  el.innerHTML = `
    ...
    <div class="prog-sec">
      <div class="ps-h">Job log <span class="dim sm">&mdash; most recent, running total</span></div>
      ${logHTML}
    </div>`;
  return el;
}

function jobLogPane(s) {
  const el = document.createElement("section");
  el.className = "card joblog-page";
  const L = window.GasLogic;
  const log = L.runningLog(data);
  const entries = log.filter((r) => r.kind !== "base");
  const name = (data.name || "").trim();

  const rows = log.map((r) => {
    if (r.kind === "base") {
      return `<div class="jlt-row base">
        <span class="jlt-n">&mdash;</span>
        <span class="jlt-d">&mdash;</span>
        <span class="jlt-desc"><em>Starting hours &mdash; before this app</em></span>
        <span class="jlt-eng">&mdash;</span>
        <span class="jlt-h">${trimNum(r.h, 1)} h</span>
        <span class="jlt-t">${trimNum(r.running, 1)} h</span>
      </div>`;
    }
    const engText = r.engineer ? engineerLabel(r.engineer) : "&mdash;";
    return `<div class="jlt-row ${r.kind === "job" ? "wu" : ""}">
      <span class="jlt-n">${r.n}</span>
      <span class="jlt-d">${esc(fmtDate(r.date, { day: "2-digit", month: "short", year: "numeric" }))}</span>
      <span class="jlt-desc">${r.note ? esc(r.note) : `<span class="dim">&mdash;</span>`}${r.kind === "job" ? ` <span class="jlt-tag">write-up</span>` : ""}</span>
      <span class="jlt-eng">${engText}</span>
      <span class="jlt-h">${trimNum(r.h, 1)} h</span>
      <span class="jlt-t">${trimNum(r.running, 1)} h</span>
    </div>`;
  }).join("");
  ...
  const tableHTML = log.length
    ? `<div class="jlt-wrap">
        <div class="jlt-row jlt-head">
          <span class="jlt-n">#</span><span class="jlt-d">Date</span><span class="jlt-desc">Description</span>
          <span class="jlt-eng">Engineer</span><span class="jlt-h">Hours</span><span class="jlt-t">Running total</span>
        </div>
        ${rows}
      </div>`
    : `<p class="dim">Nothing logged yet &mdash; entries you add on the Hours and Jobs tabs will appear here, oldest first, with a running total.</p>`;

  el.innerHTML = `
    <div class="jlt-top">
      <div>
        <h3>Job log</h3>
        <p class="dim sm">Every hours and write-up entry, oldest first, numbered, with a running hours total &mdash;
        a chronological register of the whole portfolio${name ? ` for ${esc(name)}` : ""}.</p>
      </div>
    </div>

    <div class="jlt-stats">
      ${tile2("Entries", entries.length, "logged")}
      ${tile2(trimNum(s.total, 1) + " h", "total", "of " + s.goal + " h goal")}
      ${tile2(trimNum(s.toRequired, 1) + " h", "to pass mark", s.required + " h required")}
      ${tile2(trimNum(s.toGoal, 1) + " h", "to goal", s.goal + " h goal")}
    </div>
...
function tile2(v, k, sub) {
  return `<div class="htile"><div class="ht-k">${k}</div><div class="ht-v">${v}</div><div class="ht-s">${sub || ""}</div></div>`;
}
...
function fmtDate(iso, opts) {
  const dt = parseISO(iso);
  return isNaN(dt) ? iso : dt.toLocaleDateString("en-GB", opts || { weekday: "short", day: "numeric", month: "short" });
}
...
function settingsPane() {
  ...
  const engRows = (d.engineers || []).map((e, i) => {
    ...
    const bits = [e.regNo && `reg ${esc(e.regNo)}`, e.company && esc(e.company),
      ...
      jobs ? `${jobs} write-up${jobs === 1 ? "" : "s"}` : null,
      hoursN ? `${hoursN} hours entr${hoursN === 1 ? "y" : "ies"}` : null].filter(Boolean).join(" &middot; ");
  ...
  el.innerHTML = `
    ...
    <div class="row-links">
      <button type="button" id="s_wizard">run setup again</button>
      <button type="button" id="s_example">load example data</button>
      <button type="button" id="s_export">export my data</button>
      <label class="filelink" for="s_import">import data<input type="file" class="sr-only" id="s_import" accept="application/json"></label>
      <button type="button" id="s_reset" class="danger">reset everything</button>
    </div>
    <div class="upsec">
...
function helpPane() {
  ...
  el.innerHTML = `
    ...
    <h4>Write-ups tab</h4>
    <p>Your 14 unassisted jobs (5 / 5 / 4, all editable in Settings). "Unassisted" means
    <b>you</b> carried out the hands-on work &mdash; a Gas&nbsp;Safe registered engineer still
    has to be present and responsible on site, same as for assisted hours. Pick the type, the
...
function wire(s) {
  ...
  app.querySelectorAll("[data-hubadd]").forEach((b) =>
    b.addEventListener("click", async () => {
      ...
      b.disabled = false;
      if (!r) { toast("couldn't add those photos"); return; }
      if (r.copied > 0) { toast(`${r.copied} photo${r.copied === 1 ? "" : "s"} added`); refreshHubFolders(); }
      else if (r.failed > 0) toast("couldn't copy those photos — check they're still where you picked them from");
      // else: dialog was just cancelled, nothing to say
    }));
  ...
  if (tab === "settings") {
    ...
    if (!window.api.pickBackupFolder) {
      ...
      bPick.onclick = async () => {
        ...
        if (!r) { toast("couldn't set that folder"); return; }
        if (r.ok) { paintBackup(r); toast("backup folder set"); }
        else if (r.error) toast("couldn't use that folder — " + r.error);
      };
      bOff.onclick = async () => {
    ...
    if (!window.api.pickHubFolder) {
      ...
      hPick.onclick = async () => {
        ...
        if (!r) { toast("couldn't set that folder"); return; }
        if (r.ok) { paintHub(r.dir); refreshHubFolders(); toast(r.created ? "Gas Portfolio folder set up" : "folder linked"); }
        else if (r.error) toast("couldn't use that folder — " + r.error);
      };
      hOff.onclick = async () => {
    ...
    if (step === 3) {
      ...
      if (!window.api || !window.api.pickBackupFolder) {
        ...
        pick.onclick = async () => {
          const r = await window.api.pickBackupFolder().catch(() => null);
          if (!r) { toast("couldn't set that folder"); return; }
          if (r.ok) st.textContent = "backing up to " + r.dir;
          else if (r.error) toast("couldn't use that folder — " + r.error);
        };
      }
    }
