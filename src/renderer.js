...
function buildReport(d, s) {
  ...
  const engRows = engineers.map((e) => {
    ...
    return row([esc(e.name), meta || "&mdash;", esc(e.categories || "&mdash;"), jobs, hoursN]);
  }).join("");
  const hourRows = [...d.hours].sort((a, b) => (a.date < b.date ? -1 : 1))
    .map((r) => `<tr><td>${r.date}</td><td>${(r.h ?? "") + " h"}</td><td>${esc(r.engineer || "&mdash;")}</td><td class="note">${r.note ? esc(r.note) : "&mdash;"}</td></tr>`).join("")
